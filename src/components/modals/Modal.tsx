            ...
            <div className='relative w-full md:w-4/6 lg:w-3/6 xl:w-2/5 my-6 mx-auto h-full lg:h-auto md:h-auto'>
                <div className={`translate duration-300 h-full ${showModal ? 'translate-y-0' : 'translate-y-full'} ${showModal ? 'opacity-100' : 'opacity-0'} `}>
                    <div className='translate h-full lg:h-auto md:h-auto border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white dark:bg-neutral-900 outline-none focus:outline-none'>
                        ...
                        {/* Modal Header */}
                        <div className='flex items-center p-6 rounded-t relative border-b-[1px] border-neutral-200 dark:border-neutral-700'>
                            <button onClick={handleClose} className='p-1 border-0 hover:opacity-70 transition absolute left-4 text-black dark:text-white'>
                                <IoMdClose size={18} />
                            </button>
                            <div className="text-lg font-semibold mx-auto text-center text-black dark:text-white">
                                {title}
                            </div>
                        </div>
